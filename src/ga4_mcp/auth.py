...
then exposes the bearer token for downstream GA4 API calls.
"""
...
class GA4TokenVerifier(TokenVerifier):
    ...
    bearer token – used by GA4 API clients downstream.
    """
    ...
    async def verify_token(self, token: str) -> AccessToken:
        ...
        if GA4_SCOPE not in scopes:
            raise AuthenticationError(
                f"Token is missing required scope: {GA4_SCOPE}. "
...
def create_auth_provider(base_url: str) -> RemoteAuthProvider:
    ...
    MCP clients (e.g. Claude.ai) use the protected resource metadata this
    exposes to discover that tokens must come from accounts.google.com
    with the analytics.readonly scope.
    """
    return RemoteAuthProvider(
        token_verifier=GA4TokenVerifier(),
        authorization_servers=[AnyHttpUrl(_GOOGLE_AUTH_SERVER)],
        base_url=base_url,
        scopes_supported=[GA4_SCOPE],
        resource_name="Google Analytics 4 MCP Server",
    )
